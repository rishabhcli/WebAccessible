# Sponsor Implementation Documentation
...
Do not call a sponsor integrated because a package, credential, or local mock exists. A claim requires a demonstrated live path, its named evidence artifact, and a visible failure state when that path is unavailable. This rule applies to the extension, backend, Streamlit view, recorded backup demo, submission copy, and pitch.
